...
const createArticleRequest = [
    ...
    check('video').custom(value => (value ? adminHelper.validateYouTubeUrl(value) : true)).withMessage('Video không đúng định dạng Youtube'),
    ...
    check('address').not().isEmpty().withMessage('Địa chỉ không được bỏ trống'),
    check('area').custom((value) => {
        if (value) {
    ...
    check('price[value]').custom((value) => {
        if (value) {
            value = parseInt(value, 10);
            return value > 0;
        }
